...
public class LocationPollerService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		toBroadcast.putExtra("username", username);
	...
	private class PollerThread extends WakefulThread {
		...
		private LocationListener listener = new LocationListener() {
			/**
			 * If we get a fix, get rid of the timeout condition, then attach
			 * the location as an extra (EXTRA_LOCATION) on the Intent,
			 * broadcast it, then exit the polling loop so the thread
			 * terminates.
			 */
			public void onLocationChanged(Location location) {
				...
				String username = toBroadcast.getStringExtra("username");
				toBroadcast.putExtra("username", username);
		 ...
		 * not wait forever for a location. When a timeout occurs, broadast an
		...
		protected void onPreExecute() {
			onTimeout = new Runnable() {
				public void run() {
					...
					toBroadcast.putExtra(LocationPoller.EXTRA_LOCATION, locMgr.getLastKnownLocation(LocationPoller.EXTRA_PROVIDER));
					sendBroadcast(toBroadcast);
					quit();
				}
			};
